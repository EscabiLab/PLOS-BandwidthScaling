...
%             structure contains the output of COCHLEOGRAM. This is used to
...
%             .S        : Spectrotemporal envelope (see COCHLEOGRAM)
%
% (C) Monty A. Escabi, July 2008 (Edit June 2009, Nov 2009, 2018)
%
function [RipSpec]=ripplespec(data,Fs,dX,dFm,f1,fN,Fm,OF,Norm,GDelay,dis,ATT)
...
if ~isstruct(data)
    ...
    %Temporal Down Sampling Factor - see Cochleogram.m
    DF=ceil(Fs/2/Fm/OF);

    %Normalizing Sound for Unit Variance
    data=data/std(data);

    %Generating Cochleogram
    [data]=cochleogram(data,Fs,dX,f1,fN,Fm,OF,Norm,dis,ATT);
end
taxis=data.taxis;
faxis=data.faxis;
S=data.S;
...
if strcmp(GDelay,'y') & isfield(data,{'Sc'})    %Corrected cochleogram is stored in 'data'

    S=data.Sc;
    taxis=(0:size(data.Sc,2)-1)*(taxis(2)-taxis(1));

elseif strcmp(GDelay,'y')                       %Corrected cochleogram is not stored in 'data'
